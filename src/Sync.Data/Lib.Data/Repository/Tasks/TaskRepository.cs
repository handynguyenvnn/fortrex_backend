...
namespace Lib.Data.Repository.Tasks
{
    public class TaskRepository : BaseRepository
    {
        ...
        public void ClearCache(string key)
        {
            ...
            {
                memory.ClearKeyFor(key);
            }
        }
        public decimal GetCoinPrice(string pairname, int timeClose)
        {
            var cacheKey = CacheKeyManager.SettingValueCache.Cache__GetUSDByBTC;
            return CacheExtensions.Get(cacheKey, TimeSpan.FromSeconds(10), () =>
            {
                try
                {
                    return 1;
        ...
        public int Setting_UpdateValue(string name, string value)
        {
            var map = new IntegerResultSetMapper();
            var query = _db.CreateSprocAccessor("Setting_UpdateValue", map);
            return query.Execute(name, value).FirstOrDefault();
        }
        ...
        public int User_Branch_Balance_VOL_Update(int uid, decimal bonus)
        {
            var map = new IntegerResultSetMapper();
        ...
        public void KLinesCandlestick_Ins(KlineCandlesticks candles)
        {
            var map = new IntegerResultSetMapper();
            _db.CreateSprocAccessor("CandlestickData_Ins", map).Execute(
               candles.Open,
               candles.High,
               candles.Low,
               candles.VolumeFrom,
               ...
               candles.PriceChangePercent,
               candles.Id).FirstOrDefault();
        }
        ...
        public int Ger_Server_Time_Second()
        {
            var map = new IntegerResultSetMapper();
            return _db.CreateSprocAccessor("ServerGetTime", map).Execute().FirstOrDefault();
        }

        public List<RootTradeModel> Get_Root_Trade(string pairname)
        {
            var map = NewsMapBuilder<RootTradeModel>.BuildAllProperties();
            return _db.CreateSprocAccessor("Get_Root_Trade", map).Execute(pairname).ToList();
        }

        #region Random volume
        public int Random_VolumeBuySell_Update(int b, int s)
        {
            var map = new IntegerResultSetMapper();
            var query = _db.CreateSprocAccessor("Random_VolumeBuySell_Update", map);
            return query.Execute(b,s).FirstOrDefault();
        }

        public RobotModel Ticker_Change_Last_Price(string pair)
        {
            var map = NewsMapBuilder<RobotModel>.BuildAllProperties();
            var query = _db.CreateSprocAccessor("Ticker_Change_Last_Price_New", map);
            return query.Execute(pair).FirstOrDefault();
        }

        public decimal Calculator_TradeWin(string localsymbol, InterventionSystem isys, RobotModel priceModel, int stopLoss)
        {
            if (isys == InterventionSystem.FAIR)
            {
                var rootTrade = Get_Root_Trade(localsymbol);
                int count = rootTrade.Count;
                if(count > 0)
                {
                    decimal sumRight = rootTrade.Sum(x => x.TradeRight);
                    decimal sumLeft = rootTrade.Sum(x => x.TradeLeft);
                    if(priceModel.OpenPrice <= priceModel.LastPrice) // up win
                    {
                        decimal totalTra = sumRight + sumRight * 90 / 100;
                        if(totalTra - sumLeft > stopLoss)
                        {
                            Setting_UpdateValue(Constants.INTERVENTION_SYSTEM, ((int)InterventionSystem.SMALL_WIN).ToString());
                        }
                    }
                    else //if down win
                    {
                        decimal totalTra = sumLeft + sumLeft * 90 / 100;
                        if (totalTra - sumRight > stopLoss)
                        {
                            Setting_UpdateValue(Constants.INTERVENTION_SYSTEM, ((int)InterventionSystem.SMALL_WIN).ToString());
                        }
                    }
            ...
            if (isys == InterventionSystem.CROWD_WIN)
            {
                var rootTrade = Get_Root_Trade(localsymbol);
                int count = rootTrade.Count;
                if (count > 0)
                {
                    ...
                    int userRight = rootTrade.Where(x => x.UserRignt==1).Count();
                    int userLeft = count - userRight;
                    if (userRight == userLeft)
                    {
                        return 0;
                    }
                    if (userRight > userLeft) //right win
                    {
                        decimal totalTra = sumRight + sumRight * 90 / 100;
                        if (totalTra - sumLeft > stopLoss)
                        {
                        ...
                        if (priceModel.OpenPrice <= priceModel.LastPrice)
                        {
                            return priceModel.LastPrice * (decimal)0.01 / 100; //lastPrice + voi khoang nay
                        }
                        else
                        {
                            var addNew = priceModel.OpenPrice - priceModel.LastPrice;
                            return addNew + addNew * (decimal)0.01 / 100;
                        }
                    }
                    else // left win
                    {
                        ...
                        if (priceModel.OpenPrice >= priceModel.LastPrice)
                        {
                            return (-1) * (priceModel.LastPrice * (decimal)0.01 / 100); //lastPrice + voi khoang nay
                        }
                        else
                        {
                            var addNew = priceModel.LastPrice - priceModel.OpenPrice;
                            return (-1) * (addNew + addNew * (decimal)0.01 / 100);
                        }
                    }
                }
                return 0;
            ...
            if(isys == InterventionSystem.SMALL_WIN)
            {
                var rootTrade = Get_Root_Trade(localsymbol);
                int count = rootTrade.Count;
                if (count > 0)
                {
                    ...
                    if (sumRight < sumLeft) // ben right win
                    {
                        if (priceModel.OpenPrice <= priceModel.LastPrice)
                        {
                            return priceModel.LastPrice * (decimal)0.01 / 100; //lastPrice + voi khoang nay
        ...
        #region Tools Balance Order
        public List<OpeningOrderGroupName> Tool_OpeningOrderGroupName()
        {
            var map = NewsMapBuilder<OpeningOrderGroupName>.BuildAllProperties();
            return _db.CreateSprocAccessor("Tool_OpeningOrderGroupName", map).Execute().ToList();
        }
        public List<OpeningOrders> Tool_OpeningOrders()
        {
            var map = NewsMapBuilder<OpeningOrders>.BuildAllProperties();
            return _db.CreateSprocAccessor("Tool_OpeningOrders", map).Execute().ToList();
        }
        public decimal Tool_OpeningOrders_Price_getBy_Type_Is_WIN(string PairName,string TypeWinOrLose)
        {
            var map = new DecimalResultSetMapper();
            var query = _db.CreateSprocAccessor("Tool_OpeningOrders_Price_getBy_Type_Is_WIN", map);
            return query.Execute(PairName, TypeWinOrLose).FirstOrDefault();
        }
        #endregion
